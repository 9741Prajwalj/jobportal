...
class Items extends Admin_Controller
{
    ...
    public function itemsList($group_id = null)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $main_column = array('item_name', 'bus_length','total_seats','gross_vehicle_weight','battery','capacity','temperature','motor','max_speed','quantity', 'unit_cost');
            ...
            foreach ($fetch_data as $_key => $v_items) {
                ...
                $sub_array[] = '<strong class="block">' . $item_name . '</strong>';

                $main_column = array('item_name', 'bus_length','total_seats','gross_vehicle_weight','battery','capacity','temperature','motor','max_speed','homologation','hsn_code','quantity', 'unit_cost');
            $result = array_merge($main_column, $custom_field, $action_array);

                $sub_array[] = '<strong class="block">' . $v_items->bus_length . '</strong>' . ' ' . nl2br($v_items->bus_remarks);
                $sub_array[] = '<strong class="block">' . $v_items->total_seats . '</strong>' . ' ' . nl2br($v_items->seats_remarks);
                $sub_array[] = '<strong class="block">' . $v_items->gross_vehicle_weight . '</strong>' . ' ' . nl2br($v_items->gross_vehicle_remarks);
                $sub_array[] = '<strong class="block">' . $v_items->battery . '</strong>' . ' ' . nl2br($v_items->battery_remarks);
                $sub_array[] = '<strong class="block">' . $v_items->capacity . '</strong>' . ' ' . nl2br($v_items->capacity_remarks);
                $sub_array[] = '<strong class="block">' . $v_items->temperature . '</strong>' . ' ' . nl2br($v_items->temperature_remarks);
                $sub_array[] = '<strong class="block">' . $v_items->motor . '</strong>' . ' ' . nl2br($v_items->motor_remarks);
                $sub_array[] = '<strong class="block">' . $v_items->max_speed . '</strong>' . ' ' . nl2br($v_items->max_speed_remarks);
                // $sub_array[] = '<strong class="block">' . $v_items->homologation . '</strong>' . ' ' . nl2br($v_items->homologation_remarks);
    ...
    public function saved_items($id = NULL)
    {
        ...
        $data = $this->items_model->array_from_post(array('item_name', 'item_desc', 'hsn_code', 'unit_cost', 'unit_type', 'customer_group_id', 'quantity','bus_length','bus_remarks','total_seats','seats_remarks','gross_vehicle_weight','gross_vehicle_remarks','floor_height','floor_remarks','battery','battery_remarks','capacity','capacity_remarks','temperature','temperature_remarks','motor','motor_remarks','max_speed','max_speed_remarks','charging_time','charging_time_remarks','mileage','mileage_remarks','non_ac','non_ac_remarks','homologation','homologation_remarks','itms','itms_remarks'));
