...
// Update into tbl_project
    ...
    public function opportunity_details($id, $active = NULL, $op_id = NULL)
    {
        ...
        elseif ($active == 'tender') {
            $data['active'] = 10;
            $data['sub_active'] = 2;
            $data['tender_info'] = $this->db->select('*')->from('tbl_tenders')->where(array('tender_id'=>$op_id))->get()->row();
            $data['sub_metting'] = 1;
            $data['task_active'] = 1;
            $data['bugs_active '] = 1;
        } 
...
//        redirect('admin/opportunities/opportunity_details/' . $opportunities_id . '/' . '5');
    ...
    public function save_tender($opportunities_id){
        // echo "<pre>";
        // print_r($_POST);
        // exit();
        $data['opportunities_id']   = $opportunities_id;
        ...
        if(!empty($_FILES['tender_attachment']['name'])){
            ...
            if(!$this->upload->do_upload('tender_attachment')){
                ...
                redirect('admin/opportunities/opportunity_details');
            }else{
        ...
        // print_r($data);
        // exit();
        $this->db->insert('tbl_tenders',$data);
        $type = 'success';
    ...
    public function delete_tender($opportunities_id, $id)
    {

        $data['deletion_indicator'] = 1;
        $this->db->where('tender_id',$id);
        $this->db->update('tbl_tenders',$data);
        $type = 'success';
        $message = lang('tender_deleted');
        set_message($type, $message);
        redirect('admin/opportunities/opportunity_details/' . $opportunities_id . '/' . '10');
    }
    ...
    public function update_tender($opportunities_id,$id){
        // echo "<pre>";
        // print_r($_POST);
        // exit();
        $data['tender_name']        = $_POST['tender_name'];
